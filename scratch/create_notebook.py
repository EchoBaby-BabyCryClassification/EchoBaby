...
# Baby Cry Classification V2: Librosa Baseline Experiment
...
nb.cells.append(nbf.v4.new_code_cell("""\
!pip install librosa soundfile scikit-learn pandas numpy matplotlib joblib tqdm
...
def process_dataset(root_dir, csv_output):
    ...
    for f in tqdm(files, desc=f"Extracting features for {os.path.basename(root_dir)}"):
        ...
        # Wait, filename might be Belly_Pain__d3b4...wav
...
# Cell 6: Extract 5-class
nb.cells.append(nbf.v4.new_code_cell("""\
df_5class = process_dataset('/content/datasets/master_clean_evidence/majority_5class', f'{OUTPUT_DIR}/features_5class_librosa.csv')
"""))

# Cell 7: Extract Binary
nb.cells.append(nbf.v4.new_code_cell("""\
df_binary = process_dataset('/content/datasets/master_clean_evidence/majority_binary', f'{OUTPUT_DIR}/features_binary_librosa.csv')
"""))

# Cell 8: Training & Evaluation Framework
nb.cells.append(nbf.v4.new_code_cell("""\
def evaluate_models(df, task_name, output_dir):
    ...
    models = {
        ...
        'HistGradientBoosting': HistGradientBoostingClassifier(random_state=42) # HGB doesn't support class_weight natively via arg easily in some versions, but we add it. Actually, HGB supports class_weight="balanced" in newer sklearn versions, but safely we can omit class_weight for HGB or use sample_weight during fit. We will leave it as requested.
    }
    ...
    for name, clf in models.items():
        ...
        # We also want to train on the FULL dataset to save the best model later
        # For evaluation, we strictly use cross-val
        all_y_true = []
        all_y_pred = []
    ...
    plt.savefig(f'{output_dir}/model_comparison_macro_f1_{task_name}.png', bbox_inches='tight')
    plt.close()
...
# Cell 9: Run 5-class
nb.cells.append(nbf.v4.new_code_cell("""\
best_5class, df_res_5class = evaluate_models(df_5class, '5class', OUTPUT_DIR)
"""))

# Cell 10: Run Binary
nb.cells.append(nbf.v4.new_code_cell("""\
best_binary, df_res_binary = evaluate_models(df_binary, 'binary', OUTPUT_DIR)
"""))

# Cell 11: Experiment Summary
nb.cells.append(nbf.v4.new_code_cell("""\
summary_md = f\"\"\"# Baby Cry Classification V2: Experiment Summary
...
with open(r'C:\Users\ASUS\Desktop\aml_project\baby_cry_v2_librosa_baseline.ipynb', 'w') as f:
    nbf.write(nb, f)
