...
def choose_best_group_split(X, y, groups, n_splits=5):
    ...
    for fold, (train_idx, test_idx) in enumerate(sgkf.split(X, y, groups)):
        ...
        # Check group intersection (Leakage safety)
        train_groups = set(groups[train_idx])
        test_groups = set(groups[test_idx])
        intersection = train_groups.intersection(test_groups)
        assert len(intersection) == 0, "Leakage detected: train/test groups intersect!"
...
def main():
    try:
        import google.colab
        BASE_DIR = "/content/drive/MyDrive/aml_project/outputs_for_colab_clean"
    except Exception:
        BASE_DIR = r"C:\Users\ASUS\Desktop\aml_project\outputs_for_colab_clean"

    RESULTS_DIR = os.path.join(BASE_DIR, "results_clean")
    os.makedirs(RESULTS_DIR, exist_ok=True)

    print("Veriler yükleniyor...")
    X_img = np.load(os.path.join(BASE_DIR, "X_images_new.npy"))
    ...
    np.savez(os.path.join(RESULTS_DIR, "split_indices.npz"), train=train_idx, test=test_idx)
    ...
    X_tab_train = scaler.fit_transform(X_tab_train)  # Only fit on train
    X_tab_test = scaler.transform(X_tab_test)

    with open(os.path.join(RESULTS_DIR, "scaler.pkl"), "wb") as f:
        pickle.dump(scaler, f)

    print("\n[1/3] Baseline RandomForest eğitiliyor...")
    rf = RandomForestClassifier(n_estimators=300, random_state=42, class_weight="balanced", max_depth=8)
    rf.fit(X_tab_train, y_train)
    y_pred_rf = rf.predict(X_tab_test)

    print("\n[2/3] Baseline Logistic Regression eğitiliyor...")
    lr = LogisticRegression(random_state=42, class_weight="balanced", max_iter=1000)
    lr.fit(X_tab_train, y_train)
    ...
    baseline_metrics = {
        ...
        "LogisticRegression": {
            ...
            "macro_f1": float(f1_score(y_test, y_pred_lr, average="macro", zero_division=0)),
            "macro_recall": float(recall_score(y_test, y_pred_lr, average="macro", zero_division=0))
        }
    }
    ...
    with open(os.path.join(RESULTS_DIR, "classification_report_baseline.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(classification_report(y_test, y_pred_rf, target_names=[str(c) for c in le.classes_], zero_division=0))
        f.write("\n=== LOGISTIC REGRESSION ===\n")
        f.write(classification_report(y_test, y_pred_lr, target_names=[str(c) for c in le.classes_], zero_division=0))

    print("\n[3/3] Hybrid CNN + Tabular model kuruluyor...")

    classes = np.unique(y_train)
    cw = compute_class_weight(class_weight="balanced", classes=classes, y=y_train)
    class_weights = {int(cls): float(weight) for cls, weight in zip(classes, cw)}

    model = build_hybrid_model(img_shape=X_img_train.shape[1:], tab_shape=X_tab_train.shape[1], num_classes=num_classes)
    ...
    with open(os.path.join(RESULTS_DIR, "classification_report.txt"), "w", encoding="utf-8") as f:
        f.write("=== HYBRID CNN + TABULAR MODEL RESULTS (CLEAN DATASET) ===\n")
        f.write("- Accuracy yan metriktir.\n")
        f.write("- Macro F1, Macro Recall ve per-class Recall ana metriktir.\n")
        f.write("- Dataset hash-deduplicate edildi.\n")
        f.write("- group_id = MD5 hash.\n\n")
        f.write(report)
        f.write("\n\n=== HYBRID METRICS ===\n")
    ...
    print("\n" + "="*50)
    print("EĞİTİM TAMAMLANDI - SONUÇ ÖZETİ")
    print("="*50)
    print(f"Train sample count: {len(train_idx)}")
    print(f"Test sample count: {len(test_idx)}")
    ...
    print("\n--- Baseline Metrics ---")
    print(f"RandomForest: F1={baseline_metrics['RandomForest']['macro_f1']:.4f}, Acc={baseline_metrics['RandomForest']['accuracy']:.4f}")
    print(f"LogReg      : F1={baseline_metrics['LogisticRegression']['macro_f1']:.4f}, Acc={baseline_metrics['LogisticRegression']['accuracy']:.4f}")
    
    print("\n--- Hybrid Model Metrics ---")
    print(f"Macro F1    : {hybrid_metrics['macro_f1']:.4f}")
    print(f"Macro Recall: {hybrid_metrics['macro_recall']:.4f}")
    print(f"Accuracy    : {hybrid_metrics['accuracy']:.4f}")
    
    print(f"\nTüm sonuçlar şuraya kaydedildi: {RESULTS_DIR}")
    print("="*50)
